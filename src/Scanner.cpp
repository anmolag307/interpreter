...
std::vector<Token> Scanner::scanTokens() {
    ...
    for (int i = 0; i < (int)source_.size(); ++i) {
        ...
        if (cur=='\t' || cur==' ') {
            ...
            while(i+1<(int)source_.size() && (isalnum(source_[i+1]) || source_[i+1]=='_')) {
            ...
            while(i+1<(int)source_.size() && (isdigit(source_[i+1]) || source_[i+1]=='.')) {
                number+=source_[i+1];
                i++;
            }
            ...
            std::string str;
            i++;
            while(i<(int)source_.size() && source_[i]!='"') {
                if(source_[i]=='\n'){
            ...
            if(cur == '(') {
                ...
                if(i+1<(int)source_.size() && source_[i+1]=='/'){
